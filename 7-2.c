...
static const int expected = 5905;
...
static Type type(const uchar *cards) {
	...
	for (size_t i = 0; i < ASZ(faces); i++) {
		...
		int count = face_counts[faces[i]];
		multiples[count]++;
		if (count > highest_multiple) {
			highest_multiple = count;
		}
	}

	if (jokers) {
		multiples[highest_multiple]--;
		multiples[highest_multiple + jokers]++;
	}
