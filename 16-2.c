...
static int solve(const char *const input) {
	...
	for (size_t x = 0; x < cols; x++) {
		for (size_t y = 0; y < rows; y++) {
			if (x == 0) push(&starts, (Beam){x, y, R});
			if (x == cols - 1) push(&starts, (Beam){x, y, L});
			if (y == 0) push(&starts, (Beam){x, y, D});
			if (y == rows - 1) push(&starts, (Beam){x, y, U});
		}
	}
